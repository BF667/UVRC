# coding: utf-8
"""Core model loading and audio demixing utilities."""
...
def get_model_from_config(model_type: str, config_path: str):
    """Load a model from a YAML config file.

    Returns ``(model, config)`` where *config* is a ConfigDict or OmegaConf
    object matching the model type.
    """
...
def _getWindowingArray(window_size: int, fade_size: int) -> torch.Tensor:
    """Create a crossfade window for overlap-add inference."""
...
def demix_track(config, model, mix, device, pbar=False):
    """Run overlap-add demixing for non-HTDemucs models."""
    ...
    # Pad from the beginning and end to account for floating window results
        ...
        with torch.inference_mode():
            ...
            while i < mix.shape[1]:
                ...
                if len(batch_data) >= batch_size or i >= mix.shape[1]:
                    ...
                    if i - step == 0:  # First chunk — no fadein
                        window[:fade_size] = 1
                    elif i >= mix.shape[1]:  # Last chunk — no fadeout
                        window[-fade_size:] = 1
    ...
    if config.training.target_instrument is None:
        ...
        return {
            k: v
            for k, v in zip([config.training.target_instrument], estimated_sources)
        }


def demix_track_demucs(config, model, mix, device, pbar=False):
    """Run demixing for HTDemucs models."""
...
def demix(
    ...
    """Dispatch to the correct demixing function based on model type."""
